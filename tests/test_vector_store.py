...
class TestChromaVectorStore:
    ...
    def test_query_returns_relevant_docs(self, vector_store):
        ...
        docs = [
            ...
            "Trovu teaches Peace, Love, Unity, and Respect.",
            "Creen is an evil religion opposed to Trovu.",
        ]
        vector_store.insert(documents=docs)
    ...
    def test_metadata_preserved_on_insert(self, vector_store):
        ...
        docs = ["Trovu promotes peace and love."]
        metadatas = [{"source": "trovu_lore.txt", "chapter": "1"}]

        vector_store.insert(documents=docs, metadatas=metadatas)

        # Query to retrieve the document
        results = vector_store.query("Trovu", n_results=1)
        assert len(results) == 1
        assert "Trovu" in results[0]
