...
class TestRAGWrapperInitialization:
    ...
    def test_init_loads_and_embeds_files(self, tmp_path):
        ...
        (tmp_path / "doc.txt").write_text("Trovu teaches Peace, Love, Unity, Respect.")
...
class TestRAGWrapperVectorStoreIntegration:
    ...
    def test_get_relevant_context_returns_matches(self, wrapper):
        ...
        results = wrapper.get_relevant_context("What does Trovu teach?", n_results=2)

        assert len(results) > 0
        assert any("Trovu" in doc for doc in results)
    ...
    def test_vector_store_black_box(self, tmp_path):
        ...
        data_dir = tmp_path / "data"
        data_dir.mkdir()
        (data_dir / "doc.txt").write_text("Test content for Trovu.")
...
class TestRAGWrapperChat:
    ...
    def test_chat_retrieves_context(self, wrapper):
        ...
        with patch("rag_wrapper.wrapper.litellm.completion") as mock_completion:
            ...
            wrapper.chat(session_id="test", message="What is Trovu?")
...
class TestRAGWrapperEndToEnd:
    ...
    def test_full_rag_pipeline(self, tmp_path):
        ...
        data_dir = tmp_path / "data"
        data_dir.mkdir()
        lore_file = data_dir / "trovu_lore.txt"
        lore_file.write_text("""
        Trovu is the one true faith.
        Its principles are Peace, Love, Unity, and Respect.
        Mike is the First Disciple.
        """)
        ...
        context = wrapper.get_relevant_context("What are Trovu's principles?")
        assert len(context) > 0
        assert any("Peace" in doc or "Love" in doc for doc in context)
